...
from dp_attention_contract import PAGE_SIZE, SOFTMAX_SCALE, SWA_LEFT_WINDOW
...
def reference_attention(
    *,
    ...
    if max_query_rows is None or real_q_rows <= max_query_rows:
        checked_rows = torch.arange(real_q_rows, device=q.device, dtype=torch.long)
    else:
        checked_rows = (
            torch.linspace(0, real_q_rows - 1, steps=max_query_rows, device=q.device)
            .round()
            .to(torch.long)
            .unique()
        )
    ...
    for request_id, (q_len, kv_len) in enumerate(
        ...
        request_rows = checked_rows[
            (checked_rows >= q_offset) & (checked_rows < q_offset + q_len)
        ] - q_offset

        for sample_start in range(0, request_rows.numel(), query_chunk_size):
            local_rows = request_rows[sample_start : sample_start + query_chunk_size]
            if local_rows.numel() == 0:
                continue
            query = request_q.index_select(0, local_rows).float()
            # [Q,H,D] x [K,H,D] -> [H,Q,K]
            logits = torch.einsum("qhd,khd->hqk", query, keys) * SOFTMAX_SCALE
            query_positions = prefix_len + local_rows
            ...
            output.index_copy_(
                0, q_offset + local_rows, chunk_output.to(output.dtype)
            )

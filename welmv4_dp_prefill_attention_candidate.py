...
@triton.jit
def _full_dp_padded_token_page_update(
    acc,
    l_i,
    m_i,
    q,
    ...
    """Update one causal query token held in a padded M16 head tile."""
    qk = tl.dot(q, k_t) * scale
    mask = head_mask[:, None] & key_mask[None, :]
...
@triton.jit
def _full_grouped_rows_page_update(
    acc,
    l_i,
    m_i,
    q,
...
def paged_prefill_dp_six_head_rows_kernel(
    q_ptr,
    key_cache_ptr,
    value_cache_ptr,
    o_ptr,
    ...
    """Run two proven six-head row tiles while loading each KV page once."""
    tl.static_assert(
        NUM_Q_HEADS == NUM_KV_HEADS * 12,
        "DP WeLM grouped rows require 12 Q heads per KV head",
    )
    tl.static_assert(
        MAX_Q_LEN >= 1 and MAX_Q_LEN <= 4,
        "DP WeLM grouped rows support query lengths in [1, 4]",
    )
    tl.static_assert(
        PAGE_SIZE == BLOCK_SIZE_N,
        "DP WeLM grouped rows use one page per KV tile",
    )
    GROUPED_ROWS: tl.constexpr = 6 * MAX_Q_LEN
    tl.static_assert(
        BLOCK_SIZE_M >= GROUPED_ROWS,
        "DP WeLM six-head rows must fit the Cube M tile",
    )
    tl.static_assert(
        BLOCK_SIZE_M == 16 or BLOCK_SIZE_M == 32,
        "DP WeLM six-head rows use M16/M32 Cube tiles",
    )
    ...
    for task_id in range(task_begin, task_end):
        ...
        if q_seq_len.to(tl.float32) > 0.0:
            ...
            q_ptrs1 = (
                ...
                + q_start * stride_qt
                + row_tokens[:, None] * stride_qt
                + q_head_ids1[:, None] * stride_qh
                + dim_offsets[None, :] * stride_qd
            )
            q0 = tl.load(q_ptrs0, mask=valid_rows[:, None], other=0.0)
            q1 = tl.load(q_ptrs1, mask=valid_rows[:, None], other=0.0)
                    ...
                    tl.where(
                        ...
                        tl.where(
                            ...
                            tl.where(
                                row_heads < 4,
                                sink9,
                                tl.where(row_heads < 5, sink10, sink11),
                            ),
                        ),
                    ),
                )
                m0 = tl.where(valid_rows, row_sink0, -float("inf"))
                m1 = tl.where(valid_rows, row_sink1, -float("inf"))
                l0 = tl.where(valid_rows, 1.0, 0.0).to(tl.float32)
                l1 = tl.where(valid_rows, 1.0, 0.0).to(tl.float32)
            else:
                m0 = tl.full((BLOCK_SIZE_M,), -float("inf"), tl.float32)
                m1 = tl.full((BLOCK_SIZE_M,), -float("inf"), tl.float32)
                l0 = tl.zeros((BLOCK_SIZE_M,), dtype=tl.float32)
                l1 = tl.zeros((BLOCK_SIZE_M,), dtype=tl.float32)
            acc0 = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_D), dtype=tl.float32)
            acc1 = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_D), dtype=tl.float32)
...
def paged_prefill_dp_padded_q_grouped_kernel(
    q_ptr,
    key_cache_ptr,
    value_cache_ptr,
    o_ptr,
    ...
    """Use one power-of-two M16 Cube tile for 12 Q heads per KV head."""
    tl.static_assert(
        NUM_Q_HEADS == NUM_KV_HEADS * 12,
        "padded DP WeLM prefill requires 12 Q heads per KV head",
    )
    tl.static_assert(
        MAX_Q_LEN >= 1 and MAX_Q_LEN <= 4,
        "padded DP WeLM prefill max query length must be in [1, 4]",
    )
    tl.static_assert(
        PAGE_SIZE == BLOCK_SIZE_N,
        "padded DP WeLM prefill uses one page per KV tile",
    )
    ...
    for task_id in range(task_begin, task_end):
        ...
        if q_seq_len.to(tl.float32) > 0.0:
            ...
            q0 = tl.load(q_base_ptrs, mask=head_valid[:, None], other=0.0)
            if MAX_Q_LEN >= 2:
                active1 = q_seq_len.to(tl.float32) > 1.0
                q1 = tl.load(
                    q_base_ptrs + stride_qt,
                    mask=head_valid[:, None] & active1,
                    other=0.0,
                )
            if MAX_Q_LEN >= 3:
            ...
            if SINK_ENABLED:
                ...
                m_init = tl.full((16,), -float("inf"), tl.float32)
                l_init = tl.zeros((16,), dtype=tl.float32)

            m0 = m_init
            l0 = l_init
            acc0 = tl.zeros((16, BLOCK_SIZE_D), dtype=tl.float32)
            if MAX_Q_LEN >= 2:
                m1 = m_init
                l1 = l_init
                acc1 = tl.zeros((16, BLOCK_SIZE_D), dtype=tl.float32)
            if MAX_Q_LEN >= 3:
                m2 = m_init
                l2 = l_init
                acc2 = tl.zeros((16, BLOCK_SIZE_D), dtype=tl.float32)
            if MAX_Q_LEN >= 4:
                m3 = m_init
                l3 = l_init
                acc3 = tl.zeros((16, BLOCK_SIZE_D), dtype=tl.float32)
            ...
            for kv_block_id in range(0, num_kv_blocks):
                ...
                key_positions = kv_block_start + key_offsets
                key_valid = (
                    key_positions.to(tl.float32) < kv_seq_len.to(tl.float32)
                )
                mask0 = key_valid & (
                    key_positions.to(tl.float32) <= kv_cache_len.to(tl.float32)
                )
                acc0, l0, m0 = _full_dp_padded_token_page_update(
                    acc0, l0, m0, q0, k_t, v, mask0, head_valid, softmax_scale
                )
                if MAX_Q_LEN >= 2:
                    mask1 = active1 & key_valid & (
                        key_positions.to(tl.float32)
                        <= (kv_cache_len + 1).to(tl.float32)
                    )
                    acc1, l1, m1 = _full_dp_padded_token_page_update(
                        acc1, l1, m1, q1, k_t, v, mask1, head_valid, softmax_scale
                    )
                if MAX_Q_LEN >= 3:
                    mask2 = active2 & key_valid & (
                        key_positions.to(tl.float32)
                        <= (kv_cache_len + 2).to(tl.float32)
                    )
                    acc2, l2, m2 = _full_dp_padded_token_page_update(
                        acc2, l2, m2, q2, k_t, v, mask2, head_valid, softmax_scale
                    )
                if MAX_Q_LEN >= 4:
                    mask3 = active3 & key_valid & (
                        key_positions.to(tl.float32)
                        <= (kv_cache_len + 3).to(tl.float32)
                    )
                    acc3, l3, m3 = _full_dp_padded_token_page_update(
                        acc3, l3, m3, q3, k_t, v, mask3, head_valid, softmax_scale
                    )
...
def paged_prefill_dp_small_q_grouped_kernel(
    q_ptr,
    key_cache_ptr,
    value_cache_ptr,
    o_ptr,
    ...
    """Group two proven six-head tiles while sharing each loaded KV page."""
    tl.static_assert(
        NUM_Q_HEADS == NUM_KV_HEADS * 12,
        "DP grouped WeLM small-Q prefill requires 12 Q heads per KV head",
    )
    tl.static_assert(
        MAX_Q_LEN >= 1 and MAX_Q_LEN <= 4,
        "DP grouped WeLM small-Q prefill max query length must be in [1, 4]",
    )
    tl.static_assert(
        PAGE_SIZE == BLOCK_SIZE_N,
        "DP grouped WeLM small-Q prefill uses one page per KV tile",
    )
    ...
    for task_id in range(task_begin, task_end):
        ...
        if q_seq_len.to(tl.float32) > 0.0:
            kv_seq_len = tl.load(seqlens_kv_ptr + b_id).to(tl.int32)
            kv_cache_len = kv_seq_len - q_seq_len
            six_head_offsets = tl.arange(0, 6)
            q_head_base = kv_head_id * 12
            q_head_ids0 = q_head_base + six_head_offsets
            q_head_ids1 = q_head_base + 6 + six_head_offsets
            dim_offsets = tl.arange(0, BLOCK_SIZE_D)
            q_base_ptrs0 = (
                q_ptr
                + q_start * stride_qt
            ...
            q00 = tl.load(q_base_ptrs0)
            q10 = tl.load(q_base_ptrs1)
            if MAX_Q_LEN >= 2:
                active1 = q_seq_len.to(tl.float32) > 1.0
                q01 = tl.load(q_base_ptrs0 + stride_qt, mask=active1, other=0.0)
                q11 = tl.load(q_base_ptrs1 + stride_qt, mask=active1, other=0.0)
            if MAX_Q_LEN >= 3:
                active2 = q_seq_len.to(tl.float32) > 2.0
                q02 = tl.load(q_base_ptrs0 + 2 * stride_qt, mask=active2, other=0.0)
                q12 = tl.load(q_base_ptrs1 + 2 * stride_qt, mask=active2, other=0.0)
            if MAX_Q_LEN >= 4:
                active3 = q_seq_len.to(tl.float32) > 3.0
                q03 = tl.load(q_base_ptrs0 + 3 * stride_qt, mask=active3, other=0.0)
                q13 = tl.load(q_base_ptrs1 + 3 * stride_qt, mask=active3, other=0.0)

            if SINK_ENABLED:
                m_init0 = tl.load(
                    sinks_ptr + q_head_ids0 * stride_sink_head
                ).to(tl.float32)
                m_init1 = tl.load(
                    sinks_ptr + q_head_ids1 * stride_sink_head
                ).to(tl.float32)
                l_init = tl.full((6,), 1.0, tl.float32)
            else:
                m_init0 = tl.full((6,), -float("inf"), tl.float32)
                m_init1 = tl.full((6,), -float("inf"), tl.float32)
                l_init = tl.zeros((6,), dtype=tl.float32)

            m00 = m_init0
            l00 = l_init
            acc00 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
            m10 = m_init1
            l10 = l_init
            acc10 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
            if MAX_Q_LEN >= 2:
                m01 = m_init0
                l01 = l_init
                acc01 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
                m11 = m_init1
                l11 = l_init
                acc11 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
            if MAX_Q_LEN >= 3:
                m02 = m_init0
                l02 = l_init
                acc02 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
                m12 = m_init1
                l12 = l_init
                acc12 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
            if MAX_Q_LEN >= 4:
                m03 = m_init0
                l03 = l_init
                acc03 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
                m13 = m_init1
                l13 = l_init
                acc13 = tl.zeros((6, BLOCK_SIZE_D), dtype=tl.float32)
            ...
            for kv_block_id in range(0, num_kv_blocks):
                ...
                acc00, l00, m00 = _swa_grouped_token_page_update(
                    acc00, l00, m00, q00, k_t, v, mask0, softmax_scale
                )
                acc10, l10, m10 = _swa_grouped_token_page_update(
                    acc10, l10, m10, q10, k_t, v, mask0, softmax_scale
                )
                if MAX_Q_LEN >= 2:
                    mask1 = active1 & key_valid & (
                        key_positions.to(tl.float32)
                    ...
                    acc01, l01, m01 = _swa_grouped_token_page_update(
                        acc01, l01, m01, q01, k_t, v, mask1, softmax_scale
                    )
                    acc11, l11, m11 = _swa_grouped_token_page_update(
                        acc11, l11, m11, q11, k_t, v, mask1, softmax_scale
                    )
                if MAX_Q_LEN >= 3:
                    mask2 = active2 & key_valid & (
                        key_positions.to(tl.float32)
                    ...
                    acc02, l02, m02 = _swa_grouped_token_page_update(
                        acc02, l02, m02, q02, k_t, v, mask2, softmax_scale
                    )
                    acc12, l12, m12 = _swa_grouped_token_page_update(
                        acc12, l12, m12, q12, k_t, v, mask2, softmax_scale
                    )
                if MAX_Q_LEN >= 4:
                    mask3 = active3 & key_valid & (
                        key_positions.to(tl.float32)
                    ...
                    acc03, l03, m03 = _swa_grouped_token_page_update(
                        acc03, l03, m03, q03, k_t, v, mask3, softmax_scale
                    )
                    acc13, l13, m13 = _swa_grouped_token_page_update(
                        acc13, l13, m13, q13, k_t, v, mask3, softmax_scale
                    )
            ...
            tl.store(o_base_ptrs0, (acc00 / l00[:, None]).to(o_ptr.type.element_ty))
            tl.store(o_base_ptrs1, (acc10 / l10[:, None]).to(o_ptr.type.element_ty))
            if MAX_Q_LEN >= 2:
                tl.store(
                    o_base_ptrs0 + stride_ot,
                    (acc01 / l01[:, None]).to(o_ptr.type.element_ty),
                    mask=active1,
                )
                tl.store(
                    o_base_ptrs1 + stride_ot,
                    (acc11 / l11[:, None]).to(o_ptr.type.element_ty),
                    mask=active1,
                )
            if MAX_Q_LEN >= 3:
                tl.store(
                    o_base_ptrs0 + 2 * stride_ot,
                    (acc02 / l02[:, None]).to(o_ptr.type.element_ty),
                    mask=active2,
                )
                tl.store(
                    o_base_ptrs1 + 2 * stride_ot,
                    (acc12 / l12[:, None]).to(o_ptr.type.element_ty),
                    mask=active2,
                )
            if MAX_Q_LEN >= 4:
                tl.store(
                    o_base_ptrs0 + 3 * stride_ot,
                    (acc03 / l03[:, None]).to(o_ptr.type.element_ty),
                    mask=active3,
                )
                tl.store(
                    o_base_ptrs1 + 3 * stride_ot,
                    (acc13 / l13[:, None]).to(o_ptr.type.element_ty),
                    mask=active3,
                )
...
def paged_prefill_small_q_grouped_kernel(
    ...
    """Preserve the proven TP4 six-head grouped small-query path."""
    tl.static_assert(
        NUM_Q_HEADS == 6,
    ...
    tl.static_assert(
        NUM_KV_HEADS == 1,
        "grouped WeLM small-Q prefill requires one local KV head",
    )
    tl.static_assert(
        MAX_Q_LEN >= 1 and MAX_Q_LEN <= 4,
...
def paged_attention_prefill_impl(
    ...
    if use_dp_grouped_small_q:
        grouped_grid = (min(cube_num, batch_size * num_kv_heads),)
        paged_prefill_dp_six_head_rows_kernel[grouped_grid](
            q,
            key_cache,
            value_cache,
            o,

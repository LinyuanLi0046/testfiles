...
* R29: keep one [8,512] contiguous last-axis reduction for positions 0..7,
  ...
  The two extra rank-1 reductions made large M about 27% slower, so later full
  runs retain R29 as an explicit R27 fallback.
* R30: keep R27's single [16,512] contiguous tile, but reduce the matching
  original expert ID in FP32 instead of the MMQ rank in INT32.  The fast path
  additionally rejects a tie across positions 9/10, making each output column
  strictly one-hot; IDs 0..511 are exactly representable.  Duplicate rows use
  the unchanged exact fallback.  A5 made this FP32 reduction about 30% slower,
  so later full runs retain R30 as an explicit R27 fallback.
* R31: replace only R27's full 512-value ``al.sort`` with the public standard
...
* R32: keep R27's full hardware value sort and [16,512] last-axis layout, but
  ...
  A5 still scalar-lowered rank-2 argmax to about 34.3 ms at M=16384, so later
  full runs retain R32 as an R27 fallback.
* R33: retain R27's [16,512] INT32 MMQ-rank candidates, but use public
  ``tl.min(..., return_indices=True)`` on the last axis.  The paired reduction
  directly returns the original expert column and removes rank inversion while
  keeping one reduction per row; duplicate rows use the exact R27 fallback.
  It was exact but scalar-lowered to about 37.1 ms at M=16384, so later full
  runs retain R33 as an R27 fallback.
* R34: keep R27's exact last-axis algorithm, but store and reduce unique-path
  candidate ranks as INT16.  MMQ ranks 0..511 and sentinel 513 are exactly
  representable; the reduced result is widened to INT32 before the unchanged
  inverse mapping.  Duplicate rows keep R27's INT32 fallback.
  It was exact but about 1% slower on the two large prefill shapes, so later
  full runs retain R34 as an R27 fallback.
* R35: on R27's contiguous [16,512] last-axis tile, reduce the matching INT32
...
* R36: keep the exact R27 kernel and dispatch, but enable the A5 compiler's
  ...
  It was exact but improved large prefill by only about 0.3% and regressed the
  first large boundary, so later full runs retain R36 as an R27 fallback.
* R37: keep the exact accepted dispatch and enable ``unit_flag`` only for the
...
* R42: keep R27's sort, row partition, bias schedule and exact duplicate
  ...
  It was exact but neutral on decode and slightly slower for large prefill, so
  later full runs retain R42 as an R27 fallback.
...
VARIANT_DESCRIPTIONS = {
    ...
    "r29_eight_plus_rank1_tail": (
        "rejected rank-1 tail split; exact R27 fallback"
    ),
    "r30_last_axis_direct_id": (
    ...
    "r32_last_axis_priority_argmax": (
        "rejected rank-2 argmax; exact R27 fallback"
    ),
    "r33_last_axis_paired_min_index": (
        "rejected paired min-index; exact R27 fallback"
    ),
...
def _last_axis_rank_sum_select_store_row(
    ...
    if ~has_duplicate:
        matches = top_values[:, None] == routing_scores[None, :]
        rank_candidates = matches.to(tl.int32) * tie_rank[None, :]
        if USE_MAX:
            fast_selected_rank = tl.max(rank_candidates, axis=1)
        else:
            fast_selected_rank = tl.sum(rank_candidates, axis=1)
        fast_lane = fast_selected_rank >> 4
        fast_local = fast_selected_rank & 15
        selected_ids = (
            ((fast_local >> 2) << 7)
...
@triton.jit
def _last_axis_paired_min_index_select_store_row(
    scores_ptr,
    weights_ptr,
    ids_ptr,
    row,
    ...
    """R27 unique path using one paired last-axis min-with-index."""
    scores = tl.load(scores_ptr + row * _JIT_NUM_EXPERTS + offs)
    scores = tl.where(scores == scores, scores, -float("inf"))
    routing_scores = scores + bias
    sorted_routing = al.sort(routing_scores, dim=-1, descending=True)
    ...
    if ~has_duplicate:
        matches = top_values[:, None] == routing_scores[None, :]
        candidate_ranks = tl.where(
            matches,
            tie_rank[None, :],
            _JIT_NUM_EXPERTS + 1,
        )
        _, fast_selected_ids = tl.min(
            candidate_ranks,
            axis=1,
            return_indices=True,
            return_indices_tie_break_left=True,
        )
        # Reduction columns are the original expert IDs.  Lanes 10--15 are
        # not stored, but sanitize them before the unmasked rank-1 gather.
        selected_ids = tl.where(
            out_lanes < _JIT_TOPK,
            fast_selected_ids.to(tl.int32),
            0,
        )
    else:
        invalid_rank = _JIT_NUM_EXPERTS + 1
...
@triton.jit
def _last_axis_int16_rank_select_store_row(
    scores_ptr,
    weights_ptr,
    ids_ptr,
    row,
    ...
    """R27 unique path with exact INT16 candidate-rank reduction."""
    scores = tl.load(scores_ptr + row * _JIT_NUM_EXPERTS + offs)
    scores = tl.where(scores == scores, scores, -float("inf"))
    routing_scores = scores + bias
    sorted_routing = al.sort(routing_scores, dim=-1, descending=True)
    ...
    if ~has_duplicate:
        ...
        fast_selected_rank = tl.min(candidate_ranks, axis=1).to(tl.int32)
        fast_lane = fast_selected_rank >> 4
        fast_local = fast_selected_rank & 15
        selected_ids = (
            ((fast_local >> 2) << 7)
...
@triton.jit
def _last_axis_direct_int32_id_select_store_row(
    scores_ptr,
    weights_ptr,
    ids_ptr,
    row,
    ...
    """R27 tile with one-hot last-axis INT32 original-ID min."""
    scores = tl.load(scores_ptr + row * _JIT_NUM_EXPERTS + offs)
    scores = tl.where(scores == scores, scores, -float("inf"))
    routing_scores = scores + bias
    sorted_routing = al.sort(routing_scores, dim=-1, descending=True)
    ...
    selected_ids = tl.zeros((_JIT_OUTPUT_WIDTH,), dtype=tl.int32)
    if ~has_duplicate:
        output_columns = out_lanes < _JIT_TOPK
        matches = (top_values[:, None] == routing_scores[None, :]) & (
            output_columns[:, None]
        )
        selected_ids = tl.min(
...
@triton.jit
def _last_axis_direct_id_select_store_row(
    scores_ptr,
    weights_ptr,
    ids_ptr,
    row,
    ...
    """R27 tile with an exact FP32 original-ID reduction."""
    scores = tl.load(scores_ptr + row * _JIT_NUM_EXPERTS + offs)
    scores = tl.where(scores == scores, scores, -float("inf"))
    routing_scores = scores + bias
    sorted_routing = al.sort(routing_scores, dim=-1, descending=True)
    ...
    next_lanes = tl.minimum(out_lanes + 1, _JIT_OUTPUT_WIDTH - 1)
    next_values = tl.gather(top_values, next_lanes, 0)
    # Include the 9/10 boundary.  Then every k<10 in this branch matches
    # exactly one expert even when the sorted suffix contains repeated -inf.
    adjacent_duplicate = (top_values == next_values) & (
        out_lanes.to(tl.float32) < 10.0
    )
    has_duplicate = tl.sum(adjacent_duplicate.to(tl.float32), axis=0) > 0.0
...
@triton.jit
def _partial_top16_last_axis_select_store_row(
    scores_ptr,
    weights_ptr,
    ids_ptr,
    row,
    ...
    """R27 exact recovery after public partial Top-16 value selection."""
    scores = tl.load(scores_ptr + row * _JIT_NUM_EXPERTS + offs)
    scores = tl.where(scores == scores, scores, -float("inf"))
    routing_scores = scores + bias
    top_values = tl.topk(
...
@triton.jit
def _priority_order_rank_min_select_store_row(
    scores_ptr,
    weights_ptr,
    ids_ptr,
    row,
    ...
    """R27 unique path with MMQ-priority columns as the rank axis."""
    scores = tl.load(scores_ptr + row * _JIT_NUM_EXPERTS + offs)
    scores = tl.where(scores == scores, scores, -float("inf"))
    routing_scores = scores + bias
    sorted_routing = al.sort(routing_scores, dim=-1, descending=True)
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r43_last_axis_rank_max_medium_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r43_last_axis_rank_max_large_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r45_priority_order_rank_min_medium_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
    ...
    for row in tl.range(row_start, row_end):
        bias = tl.load(bias_ptr + offs)
        _priority_order_rank_min_select_store_row(
            scores_ptr, weights_ptr, ids_ptr, row, offs, tie_rank, bias
        )


@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r45_priority_order_rank_min_large_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r30_last_axis_direct_id_medium_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
    ...
    for row in tl.range(row_start, row_end):
        bias = tl.load(bias_ptr + offs)
        _last_axis_direct_id_select_store_row(
            scores_ptr, weights_ptr, ids_ptr, row, offs, tie_rank, bias
        )


@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r30_last_axis_direct_id_large_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r31_partial_top16_medium_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
    ...
    for row in tl.range(row_start, row_end):
        bias = tl.load(bias_ptr + offs)
        _partial_top16_last_axis_select_store_row(
            scores_ptr, weights_ptr, ids_ptr, row, offs, tie_rank, bias
        )


@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r31_partial_top16_large_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r33_last_axis_paired_min_index_medium_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
    ...
    for row in tl.range(row_start, row_end):
        bias = tl.load(bias_ptr + offs)
        _last_axis_paired_min_index_select_store_row(
            scores_ptr, weights_ptr, ids_ptr, row, offs, tie_rank, bias
        )


@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r33_last_axis_paired_min_index_large_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r34_last_axis_int16_rank_medium_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
    ...
    for row in tl.range(row_start, row_end):
        bias = tl.load(bias_ptr + offs)
        _last_axis_int16_rank_select_store_row(
            scores_ptr, weights_ptr, ids_ptr, row, offs, tie_rank, bias
        )


@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r34_last_axis_int16_rank_large_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r35_last_axis_direct_int32_id_medium_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
    ...
    for row in tl.range(row_start, row_end):
        bias = tl.load(bias_ptr + offs)
        _last_axis_direct_int32_id_select_store_row(
            scores_ptr, weights_ptr, ids_ptr, row, offs, tie_rank, bias
        )


@triton.jit(do_not_specialize=["rows_per_program", "extra_rows"])
def _r35_last_axis_direct_int32_id_large_kernel(
    scores_ptr,
    bias_ptr,
    weights_ptr,
    ids_ptr,
...
class ModelNew(torch.nn.Module):
    ...
    def bind_kernel(
        ...
        if variant == "r29_eight_plus_rank1_tail":
            # R29's extra rank-1 reductions regressed large M by ~27%.  Keep
            # the historical row measurable as the exact accepted R27 path.
            grid, rows_per_program, extra_rows = self._partition(m)
            if m <= self.num_vector_cores:
                kernel = _r7_vector_tie_small_kernel
        ...
        if variant == "r30_last_axis_direct_id":
            # R30's last-axis FP32 ID reduce regressed large M by ~30%.  Keep
            # the historical row measurable as the exact accepted R27 path.
            grid, rows_per_program, extra_rows = self._partition(m)
            if m <= self.num_vector_cores:
                kernel = _r7_vector_tie_small_kernel
        ...
        if variant == "r32_last_axis_priority_argmax":
            # R32's rank-2 argmax scalar-lowered to ~34.3 ms at M=16384.
            # Keep this historical row as the accepted exact R27 path.
            grid, rows_per_program, extra_rows = self._partition(m)
            if m <= self.num_vector_cores:
                kernel = _r7_vector_tie_small_kernel
            elif m <= R6_DISPATCH_CUTOFF_M:
        ...
        if variant == "r33_last_axis_paired_min_index":
            # R33's paired rank-2 reduction scalar-lowered to ~37.1 ms at
            # M=16384. Keep this historical row as the accepted R27 path.
            grid, rows_per_program, extra_rows = self._partition(m)
            if m <= self.num_vector_cores:
                kernel = _r7_vector_tie_small_kernel
        ...
        if variant == "r34_last_axis_int16_rank":
            # R34 was exact but about 1% slower on large prefill. Keep this
        ...
        if variant == "r36_large_multibuffer":
            # R36 gained only ~0.3% on large prefill and regressed the first
            # large boundary. Keep this historical row as exact R27.
            grid, rows_per_program, extra_rows = self._partition(m)
            if m <= self.num_vector_cores:
                kernel = _r7_vector_tie_small_kernel
            elif m <= R6_DISPATCH_CUTOFF_M:
        ...
        if variant == "r42_last_axis_rank_sum":
            # R42 was exact but neutral/slightly slower. Keep its historical
        ...
        if variant == "r44_two_half_partial_sort":
            # R44 was exact but ~19% slower on large prefill. Keep its
            # historical row as the exact accepted R27 path.
            grid, rows_per_program, extra_rows = self._partition(m)
            if m <= self.num_vector_cores:
                kernel = _r7_vector_tie_small_kernel
...
def parse_variants(spec: str) -> list[str]:
    ...
    if normalized in ("0-33", "r0-r33"):
        return list(VARIANTS[:34])
    if normalized in ("0-32", "r0-r32"):
        return list(VARIANTS[:33])

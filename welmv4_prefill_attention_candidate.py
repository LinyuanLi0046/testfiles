...
@triton.jit
def paged_prefill_verify_split_kernel(
    q_ptr,
    key_cache_ptr,
    value_cache_ptr,
    acc_ws_ptr,
    ...
    """Phase 1: balanced partial Full verify over page-aligned KV splits."""
    GROUPED_ROWS: tl.constexpr = NUM_Q_HEADS * VERIFY_WIDTH
    tl.static_assert(
        NUM_Q_HEADS == 6,
        "split WeLM full verify requires six local Q heads",
    )
    tl.static_assert(
        PAGE_SIZE == BLOCK_SIZE_N,
        "split WeLM full verify uses one page per KV tile",
    )
    tl.static_assert(
        KV_SPLIT_PARTS == 4,
        "split WeLM full verify currently requires four KV parts",
    )
    tl.static_assert(
        BLOCK_SIZE_M >= GROUPED_ROWS,
        "split WeLM full verify must pad all token/head rows",
    )

    pid = tl.program_id(0)
    n_programs = tl.num_programs(0)
    total_tasks = batch_size * KV_SPLIT_PARTS
    task_begin = pid * total_tasks // n_programs
    task_end = (pid + 1) * total_tasks // n_programs
    for task_id in range(task_begin, task_end):
        split_id = task_id - (task_id // KV_SPLIT_PARTS) * KV_SPLIT_PARTS
        b_id = task_id // KV_SPLIT_PARTS
        q_start = tl.load(cu_q_lens_ptr + b_id).to(tl.int32)
        q_end = tl.load(cu_q_lens_ptr + b_id + 1).to(tl.int32)
        q_seq_len = q_end - q_start
        if q_seq_len.to(tl.float32) > 0.0:
            ...
            kv_start = min(split_id * chunk_size, kv_seq_len)
...
@triton.jit
def paged_prefill_verify_split_reduce_kernel(
    acc_ws_ptr,
    lse_ws_ptr,
    o_ptr,
    cu_q_lens_ptr,
    ...
    for b_id in range(b_begin, b_end):
        q_start = tl.load(cu_q_lens_ptr + b_id).to(tl.int32)
        q_end = tl.load(cu_q_lens_ptr + b_id + 1).to(tl.int32)
        if (q_end - q_start).to(tl.float32) > 0.0:
            row_ids = tl.arange(0, BLOCK_SIZE_M)
            valid_rows = row_ids.to(tl.float32) < GROUPED_ROWS
            row_tokens = row_ids // NUM_Q_HEADS
            row_heads = row_ids - row_tokens * NUM_Q_HEADS
                    ...
                    tl.where(
                        ...
                        tl.where(
                            ...
                            tl.where(
                                row_heads_f32 < 4.0,
                                sink3,
                                tl.where(row_heads_f32 < 5.0, sink4, sink5),
                            ),
                        ),
                    ),
                )
...
def paged_attention_prefill_impl(
    ...
    if use_grouped_verify:
        ...
        # A 48-request verify batch leaves 20 of 28 Cube cores scanning two
        # complete KV sequences while eight cores scan only one.  Split each
        # sequence into four page-aligned parts so phase 1 has 192 similarly
        # sized tasks (six or seven per core), then merge the four online-
        # softmax states in a small vector reduction.  Keep this first split
        # iteration shape-specific until remote NPU measurements establish
        # where the extra workspace/reduction launch amortizes.
        use_split_verify = batch_size == 48
        if use_split_verify:
            kv_split_parts = 4
            split_tasks = batch_size * kv_split_parts
            acc_ws = torch.empty(
                (split_tasks, block_size_m, head_dim),
                dtype=torch.float32,
                device=q.device,
            )
            lse_ws = torch.empty(
                (split_tasks, block_size_m),
                dtype=torch.float32,
                device=q.device,
            )
            split_grid = (min(cube_num, split_tasks),)
            paged_prefill_verify_split_kernel[split_grid](
                q,
                key_cache,
                value_cache,
                acc_ws,
            ...
            reduce_grid = (min(vector_num, batch_size),)
            paged_prefill_verify_split_reduce_kernel[reduce_grid](
                acc_ws,
                lse_ws,
                o,
                cu_q_lens,

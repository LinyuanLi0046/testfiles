...
def _swa_paged_prefill_verify_grouped_sink_kernel(
    ...
    for b_id in range(b_begin, b_end):
        ...
        if q_seq_len.to(tl.float32) > 0.0:
            # Keep all vectors inside the request's Vector scope.  Hoisting an
            # arange above this dynamic request loop leaves it outside the
            # function generated by BiSheng's mixed Cube/Vector lowering.
            ...
            for kv_block_iter in range(
                ...
                key_offsets = tl.arange(
                    0, BLOCK_N * PAGE_AGGREGATION_NUM
                )
                is_global = (
